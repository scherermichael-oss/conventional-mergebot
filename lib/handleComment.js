...
const handleComment = async function (context) {
  ...
  if (context.payload.comment.body.toLowerCase().trim() !== '/merge') {
  ...
  const target = context.repo({ pull_number: number })

  context.payload.pull_request = (await context.github.pulls.get(target)).data
