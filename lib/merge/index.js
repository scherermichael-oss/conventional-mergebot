...
const merge = async function (context) {
  ...
  if (context.payload.comment && context.payload.comment.body.toLowerCase().trim() === '/merge') {
  ...
  try {
    ...
    try {
      if (exception.message.match(/required status (check .* is|checks are) pending./i)) {
        return
      }
    } catch {
    }
