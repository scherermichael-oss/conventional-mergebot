...
const merge = async function (context) {
  ...
  try {
    ...
    await context.github.issues.createComment(context.repo({
      body: await buildResultMessage({ context, commitMessage, ref: base.ref }),
      issue_number: number
    }))
    ...
    if (exception.message && exception.message.match(/required status (check .* is|checks are) (expected|queued|in progress)./i)) {
      return
    }
