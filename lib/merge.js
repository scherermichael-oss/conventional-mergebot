...
const merge = async function (context) {
  ...
  if (!context.payload.pull_request.mergeable) {
    ...
    if (context.payload.comment) {
      await context.github.issues.createComment(context.repo({
        body: [
          '⚠️ Unable to Merge Pull Request ⚠️',
          '',
          'This pull request is not in a mergeable state at the moment.',
          '',
          'Please resolve the problem and run the `/merge` command again.',
        ].join('\n'),
        issue_number: number
      }))
    }
  ...
  if (message.title.toLowerCase().startsWith('wip:')) {
    if (context.payload.comment) {
      ...
      await context.github.issues.createComment(context.repo({
        body: [
          '⚠️ Unable to Merge Work in Progress ⚠️',
          '',
          '"WIP:" must be removed from the title before it can be merged.',
          '',
          'Please repeat the `/merge` command afterwards.'
        ].join('\n'),
        issue_number: number
      }))
    }
  ...
  let successComment = (releaseType && base.ref === 'master') ? [
    '🎉 Pull Request Merged 🎉',
    '',
    ...
    '🎉 Pull Request Merged 🎉',
    '',
    'No type of pull request detected or not merged into `master` branch. Therefore, no release is triggered.'
  ]
  ...
  try {
    ...
    let errorTitle = '⚠️ Merging Pull Request Failed ⚠️'
    let errorReaction = 'confused'

    // If error is caused by pending checks, notify user about automatic retries
    if (errorMessage.match(/required status (check .* is|checks are) pending./i)) {
      errorMessage += '\n\nMerge will be **automatically retried** after checks are completed.'
      errorTitle = '⌛ Merging Pull Request Postponed ⌛'
      errorReaction = 'eyes'
    }
