...
const debug = require('debug')('app:checks:isMergePermitted')
...
const isMergePermitted = async function (context) {
  ...
  const level = await context.octokit.repos.getCollaboratorPermissionLevel(
  ...
  if (!level || !level.data || !['admin', 'maintain', 'write'].includes(level.data.permission)) {
    ...
    // Add reaction
    await context.octokit.reactions.createForIssueComment(context.repo({
