...
const containsPrMergeComment = require('./containsPrMergeComment')
...
const handleCheck = async function (context) {
  ...
  for (const { number } of pullRequests) {
    ...
    if (!automergeBranches.includes(branchName) && !await containsPrMergeComment(context)) {
      return debug('Branch is not in auto-merge list and no merge commit found. Skip.')
    }
